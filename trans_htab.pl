...
/^(\S+)\s+(\d+)\/(\d+)\s+(\d+)\s+(\d+).+?(\d+)\s+(\d+).+?(-?\d\S*)\s+(\d\S*)/
     ...
     # get aligned sequences ---------------------------------------------------
            ...
            && $method eq 'hmmsearch' )
        {
            ...
            #get the maximum range of hmm sequences, required for fragment model
...
_EOT_
	...
	for (@file) {
	    ...
	    &parse_hmm_hits($file, $info{db}, $info{db_proc}, $usage, $info{debug}, $info{align_output}, $info{align_file}, $info{b1_cutoff}, $info{b2_cutoff}, $info{e1_cutoff}, $info{e2_cutoff}, $info{format}, $info{quiet}, $info{noise_cutoff}, $info{trusted_cutoff});
	}
    }
    else {
	&parse_hmm_hits('', $info{db}, $info{db_proc}, $usage, $info{debug}, $info{align_output}, $info{align_file}, $info{b1_cutoff}, $info{b2_cutoff}, $info{e1_cutoff}, $info{e2_cutoff}, $info{format}, $info{quiet}, $info{noise_cutoff}, $info{trusted_cutoff});
    }
...
sub get_options {
    use strict;
    use Getopt::Std;
    use vars qw($opt_A $opt_B $opt_C $opt_D $opt_E $opt_F $opt_Z $opt_f $opt_h $opt_n $opt_q $opt_s $opt_t);
    my ($info_r, $usage) = @_;
    getopts('A:B:C:D:E:F:Zfhn:qst:') or die "Wrong input options. \n"; #get options
    die "$usage" if($opt_h);
